...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'update') {
  ...
  $newQuantity = (int)$_POST['quantidade'];

  if (isset($_SESSION['carrinhos'][$itemKey])) {
      $_SESSION['carrinhos'][$itemKey]['quantidade'] = $newQuantity;
  }
}

// Verifica se a variável 'esvaziar' está presente na URL
if (isset($_GET['esvaziar'])) {
  // Verifica se a variável de sessão 'carrinhos' está definida
  if (isset($_SESSION['carrinhos'])) {
    // Remove a variável de sessão 'carrinhos'
    unset($_SESSION['carrinhos']);
  }
}


// Verifica se a variável 'remover' está presente na URL
if (isset($_GET['remover'])) {
  // Obtém o valor da variável 'remover' da URL
  $nome = $_GET['remover'];
  // Verifica se o item está presente no array 'carrinhos' da sessão
  if (isset($_SESSION['carrinhos'][$nome])) {
    // Remove o item do array 'carrinhos'
    unset($_SESSION['carrinhos'][$nome]);
  }
}
...
<head>
  ...
  <link rel="stylesheet" href="../css/cartinho.css">
</head>
...
if (isset($_SESSION["carrinhos"])) {
    foreach ($_SESSION['carrinhos'] as $key => $value) {
        $nome = $value['nome'];
        $total = $value["quantidade"] * $value["valor"];
        $totalGeral += $total;

        echo "<tr>
            <td><img src='{$value['imagem']}' alt='{$value['nome']}' class='product-image'></td>
            <td>{$value['nome']}</td>
            <td>
                <form method='post' action='cart.php'>
                    <input type='hidden' name='item_key' value='{$key}'>
                    <input type='hidden' name='action' value='update'>
                    <input type='number' name='quantidade' value='{$value['quantidade']}' min='1'>
                    <button type='submit'>Atualizar</button>
                </form>
            </td>
            <td>R$ {$value['valor']}.00</td>
            <td>R$ $total.00</td>
        </tr>
        <tr>
            <td><a href='cart.php?remover=$nome'>Remover</a></td>
        </tr>";
    }
}
?>
        ...
        <tfoot>
          <tr>
            ...
            <td>R$ <?php echo $totalGeral; ?>.00</td>
          </tr>
        </tfoot>
      </table>
    </div>
  ...
  <!-- Botões -->
  <div class="meio d-flex flex-column flex-sm-row justify-content-between mt-3">
    <button class="custom-btn btn-success comprar mb-2 mb-sm-0">Finalizar</button>
    <a href="?esvaziar">
      <button class="custom-btn btn-danger esvaziar">Esvaziar</button>
    </a>
  </div>
